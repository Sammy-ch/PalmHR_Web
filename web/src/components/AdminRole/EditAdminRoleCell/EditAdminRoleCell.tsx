import type {
  EditAdminRoleById,
  UpdateAdminRoleInput,
  UpdateAdminRoleMutationVariables,
} from 'types/graphql'

import { navigate, routes } from '@redwoodjs/router'
import type {
  CellSuccessProps,
  CellFailureProps,
  TypedDocumentNode,
} from '@redwoodjs/web'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import AdminRoleForm from 'src/components/AdminRole/AdminRoleForm'

export const QUERY: TypedDocumentNode<EditAdminRoleById> = gql`
  query EditAdminRoleById($id: String!) {
    adminRole: adminRole(id: $id) {
      id
      createdAt
      updatedAt
      name
      adminId
    }
  }
`

const UPDATE_ADMIN_ROLE_MUTATION: TypedDocumentNode<
  EditAdminRoleById,
  UpdateAdminRoleMutationVariables
> = gql`
  mutation UpdateAdminRoleMutation(
    $id: String!
    $input: UpdateAdminRoleInput!
  ) {
    updateAdminRole(id: $id, input: $input) {
      id
      createdAt
      updatedAt
      name
      adminId
    }
  }
`

export const Loading = () => <div>Loading...</div>

export const Failure = ({ error }: CellFailureProps) => (
  <div className="rw-cell-error">{error?.message}</div>
)

export const Success = ({ adminRole }: CellSuccessProps<EditAdminRoleById>) => {
  const [updateAdminRole, { loading, error }] = useMutation(
    UPDATE_ADMIN_ROLE_MUTATION,
    {
      onCompleted: () => {
        toast.success('AdminRole updated')
        navigate(routes.adminRoles())
      },
      onError: (error) => {
        toast.error(error.message)
      },
    }
  )

  const onSave = (
    input: UpdateAdminRoleInput,
    id: EditAdminRoleById['adminRole']['id']
  ) => {
    updateAdminRole({ variables: { id, input } })
  }

  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">
          Edit AdminRole {adminRole?.id}
        </h2>
      </header>
      <div className="rw-segment-main">
        <AdminRoleForm
          adminRole={adminRole}
          onSave={onSave}
          error={error}
          loading={loading}
        />
      </div>
    </div>
  )
}
